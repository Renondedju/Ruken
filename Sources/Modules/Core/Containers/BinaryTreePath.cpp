...
{
	auto recursive_get_index = [](
		this		   const auto& in_self,
		BinaryTreePath const& 	   in_tree,
		RkUint64	   const  	   in_current_depth = 0) -> RkUint64
	{
		if (in_tree.depth == in_current_depth)
			return 0;

		const RkUint64 depth_mask   {1ULL << in_current_depth};
		const RkBool   is_bit_set   {(in_tree.path & depth_mask) == depth_mask};
		const RkUint64 contribution {1 + in_tree.GetNodeCount(in_current_depth + 1) * is_bit_set};

		return contribution + in_self(in_tree, in_current_depth + 1);
	};

	return recursive_get_index(*this);
}
