...
#include "ExecutiveSystem/CPU/Awaitables/CPUAwaitableResultStorage.hpp"
#include "ExecutiveSystem/CPU/Awaitables/CPUAwaitableExceptionStorage.hpp"
...
RkVoid CPUAwaitable<TResult, TNoexcept>::Signal() const noexcept
{
    ...
    while (true)
    {
        ...
        // Fetching the forward awaiter
        selection = &continuation->forward;
...
RkVoid CPUAwaitable<TResult, TNoexcept>::Consume() noexcept
{
    ...
    while(true)
    {
        ...
        if (selection->compare_exchange_weak(continuation, CPUContinuation::consumed, std::memory_order_acq_rel))
            selection = &continuation->forward; // Fetching the forward awaiter
    }
}
