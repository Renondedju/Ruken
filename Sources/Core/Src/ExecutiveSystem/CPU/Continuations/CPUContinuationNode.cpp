...
RkBool CPUContinuationNode::Consumed() const noexcept
{
    return head->load(std::memory_order_acquire) == consumed;
}

RkBool CPUContinuationNode::TryStartAwait() noexcept
{
    CPUContinuationNode* head_value {head->load(std::memory_order_acquire)};
...
RkBool CPUContinuationNode::TryStopAwait() noexcept
{
    ...
    if (next.load(std::memory_order_acquire) == consumed || head == nullptr)
        return false;

    // Attempting to detach from the awaited event by looking for our
    // address though the list of suspensions
    std::atomic<CPUContinuationNode*>* selection {head};
    CPUContinuationNode*			   expected  {this};
