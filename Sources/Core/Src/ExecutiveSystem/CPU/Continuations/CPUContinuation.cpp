...
RkBool CPUContinuation::TryAttach() noexcept
{
    ...
    do
    {
        ...
        forward.store(head_value, std::memory_order_release);
...
RkBool CPUContinuation::TryDetach()
{
    ...
    if (forward.load(std::memory_order_acquire) == consumed)
        return false;
    ...
    selection->store(forward.load(std::memory_order_acquire), std::memory_order_release);
