...
CPUQueue::CPUQueue(const RkSize in_size) noexcept:
...
RkVoid CPUQueue::TryConsumeJob(RkUint32 const in_max_attempts) noexcept
{
...
RkFloat CPUQueue::GetSignedConcurrencyRequest(ConcurrencyCounter const& in_concurrency, RkUint32 const in_offset) const noexcept
{
...
RkVoid CPUQueue::Yield(std::stop_token const& in_stop_token) const noexcept
{
    ZoneNamed(__tracy, static_cast<bool>(RUKEN_TRACE_SHOW_WORKER_ZONES));
...
RkVoid CPUQueue::Push(std::coroutine_handle<> in_handle) noexcept
{
    ZoneNamed(__tracy, static_cast<bool>(RUKEN_TRACE_SHOW_WORKER_ZONES));
...
RkVoid CPUQueue::PopAndRun(RkBool const in_greedy, std::stop_token const& in_stop_token) noexcept
{
    ZoneNamed(__tracy, static_cast<bool>(RUKEN_TRACE_SHOW_WORKER_ZONES));
...
RkFloat CPUQueue::ComputeOptimalConcurrency(RkUint32 const in_max_concurrency) const noexcept
{
