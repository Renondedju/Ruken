...
{
	return m_awaiter_list.load(std::memory_order_acquire) == CPUAwaiter::consumed;
}

RkBool ManualResetEvent::SignalConsume() const noexcept
{
	CPUAwaiterList*   selection {&m_awaiter_list};
	CPUAwaiter const* previous  {nullptr};
	CPUAwaiter*   	  continuation;

	if (selection->load(std::memory_order_acquire) == CPUAwaiter::consumed)
		return false;
	...
	while(true)
	{
		do
		{
			...
			if (selection->compare_exchange_strong(continuation = nullptr, CPUAwaiter::consumed, std::memory_order_acq_rel))
			{
		...
		} while(continuation == CPUAwaiter::locked);
		...
		if (selection->compare_exchange_weak(continuation, CPUAwaiter::consumed, std::memory_order_acq_rel))
		{
