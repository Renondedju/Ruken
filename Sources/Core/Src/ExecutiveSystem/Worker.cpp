#include "ExecutiveSystem/Worker.hpp"
...
Worker::Worker(std::string in_name, std::vector<JobQueue*>& in_queues) noexcept:
    m_queues {in_queues},
    m_thread {std::bind_front(&Worker::Routine, this), std::move(in_name)}
{}

RkVoid Worker::ProcessQueues(std::vector<JobQueue*> const& in_queues, std::stop_token const& in_stop_token) noexcept
{
