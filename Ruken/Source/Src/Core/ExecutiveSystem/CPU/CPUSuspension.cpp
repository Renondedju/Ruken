...
CPUSuspension::CPUSuspension(Node& in_head) noexcept:
    head {in_head}
{}

CPUSuspension::~CPUSuspension() noexcept
{
...
RkBool CPUSuspension::await_ready() const noexcept
{
    // Since we are not attached yet, we can use the head directly
    return head.load(std::memory_order_acquire) == consumed;
}

RkBool CPUSuspension::await_suspend(std::coroutine_handle<>) noexcept
{
